...
class DimensionalApp {
    ...
    initEventListeners() {
        ...
        if (infoPanel) {
            // Améliorer la détection de défilement vs clic
            let startY = 0;
            let scrolling = false;
            ...
            infoPanel.addEventListener('touchmove', (e) => {
                ...
                // Si l'élément peut défiler
                if (infoPanel.scrollHeight > infoPanel.clientHeight) {
                    scrolling = true;
                    // Marquer comme scrollable pendant un moment
            ...
            // Remplacer l'événement de clic pour éviter le déclenchement lors du défilement
            infoPanel.addEventListener('click', (e) => {
                if (scrolling) {
                    // Ne pas ouvrir si on fait juste défiler
                    scrolling = false;
                    return;
                }
                
                // Le reste du code pour gérer le clic et l'agrandissement
    ...
    onWindowResize() {
        ...
        // Ajuster la hauteur maximum du panneau d'info sur les écrans plus petits
        const infoPanel = document.getElementById('info-panel');
        if (infoPanel && window.innerWidth <= 375) {
            infoPanel.style.maxHeight = '80px';
        } else if (infoPanel && window.innerWidth <= 600) {
